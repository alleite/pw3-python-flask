...
import urllib  # vai ler a URl
import json  # vai ler o JSON
...
def init_app(app):
    ...
    def apigames(id=None):  # só entra se ^ se informar o parametro ai vai pra rota
        url = 'https://www.freetogame.com/api/games'
        response = urllib.request.urlopen(url)
        apiData = response.read()
        gameList = json.loads(apiData)
        # se id existir (pu seja, foi informado o parametro)
        if id:
            gameInfo = []
